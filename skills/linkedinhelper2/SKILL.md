...
| Warm an audience before selling | `Follow profiles`, `Like & comment posts and articles`, `Endorse 1st connections` | None | Endorsement = 1 action per profile regardless of skill count |
| Fill an event / group / company page | `Invite person to an event`, `Invite 1st connections to Group`, `Invite 1st to follow Organization` | None | Separate credit pools; Standard licence caps these at 20/24 h |
| Hygiene | `Remove 1st connections` (template); `Sent invites canceller` (`Functions` menu) | None | Withdrawn invite ⇒ cannot re-invite for up to 3 weeks |
